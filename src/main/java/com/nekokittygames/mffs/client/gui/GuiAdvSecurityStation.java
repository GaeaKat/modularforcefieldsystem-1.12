...
public class GuiAdvSecurityStation extends GuiContainer {
	...
	protected void drawGuiContainerBackgroundLayer(float f, int mouseX,
		...
		mc.renderEngine.bindTexture(new ResourceLocation("modularforcefieldsystem:textures/gui/GuiAdvSecstation.png"));
		int w = (width - xSize) / 2;
	...
	private void drawSprite(int par1, int par2, int par3, int par4,
		...
		var10.pos(par1 + 0, par2 + 18,
				...
				(par4 + 18) * 0.0078125F).endVertex();
		var10.pos(par1 + 18, par2 + 18,
				this.zLevel).tex(
				(par3 + 18) * 0.0078125F,
				(par4 + 18) * 0.0078125F).endVertex();
		var10.pos(par1 + 18, par2 + 0,
				this.zLevel).tex(
				(par3 + 18) * 0.0078125F,
				(par4 + 0) * 0.0078125F).endVertex();
		var10.pos(par1 + 0, par2 + 0,
				this.zLevel).tex(
				(par3 + 0) * 0.0078125F,
				(par4 + 0) * 0.0078125F).endVertex();
		Tess.draw();

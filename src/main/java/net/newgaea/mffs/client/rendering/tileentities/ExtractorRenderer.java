...
public class ExtractorRenderer extends TileEntityRenderer<TileExtractor> {
    public ExtractorRenderer(TileEntityRendererDispatcher rendererDispatcherIn) {
        super(rendererDispatcherIn);
    }

    @Override
    public void render(TileExtractor tileEntityIn, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn, int combinedOverlayIn) {
        matrixStackIn.push();
        ...
        fontRenderer.drawString(matrixStackIn,"WE:", offsetX - realWidth / 2, 1 + offsetY
                - realHeight / 2 + -0 * lineHeight, 1);
        fontRenderer.drawString(matrixStackIn,
                String.valueOf(tileEntityIn.getWorkDone()).concat(" % "),
                offsetX
                        + realWidth
                        / 2
                        - offsetX
                        - fontRenderer.getStringWidth(String.valueOf(
                        tileEntityIn.getWorkDone()).concat(" % ")),
                offsetY - realHeight / 2 - 0 * lineHeight, 1);
        fontRenderer.drawString(matrixStackIn,"WC left:", offsetX - realWidth / 2, 1
                + offsetY - realHeight / 2 + 1 * lineHeight, 1);
        fontRenderer.drawString(matrixStackIn,
                String.valueOf(tileEntityIn.getWorkCycle()),
                offsetX
                        + realWidth
                        / 2
                        - offsetX
                        - fontRenderer.getStringWidth(String
                        .valueOf(tileEntityIn.getWorkCycle())), offsetY
                        - realHeight / 2 + 1 * lineHeight, 1);
        fontRenderer.drawString(matrixStackIn,"FE Cap:", offsetX - realWidth / 2, 1
                + offsetY - realHeight / 2 + 2 * lineHeight, 1);
        fontRenderer.drawString(matrixStackIn,
                String.valueOf(tileEntityIn.getCapacity()).concat("%"),
                offsetX
                        + realWidth
                        / 2
                        - offsetX
                        - fontRenderer.getStringWidth(String.valueOf(
                        tileEntityIn.getCapacity()).concat("%")),
                offsetY - realHeight / 2 + 2 * lineHeight, 1);
        matrixStackIn.pop();
    }
}
